...
public
    class S27994Set01_kopia
    extends Frame {
    private final File BINARY_FILE = new File("PRIMES.bin");
    ...
    public S27994Set01_kopia(int width, int height){
        super();
        SIZE = (int)Math.pow(Math.min(width,height)*10,2);
    ...
    public void paint(Graphics g) {
        ...
        int stepsToReach = 1,
            ...
            smallerBound = (int)Math.pow(Math.min(getWidth(),getHeight()),2);
        ...
        try {
            ...
            for (int i = 1; (y >= insets().top && y <= getHeight() && x >= 0 && x <= getWidth()); i++) {
    ...
    private void writeBinaryFile(){
        try {
            ...
            for (int i = 1; i <= SIZE; i++) {
                ...
                if (newLine){
                    for (int j = i; j <= Math.min(SIZE,(int)Math.pow(2,8 * bytes)); j++) {
                        if (isPrime(j)){
                            row.add(j);
                            counter++;
                        }
